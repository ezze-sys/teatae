<div class="flex h-screen bg-gray-50 dark:bg-gray-900 overflow-hidden font-sans" x-data="{ 
    notifications: [],
...
@notify.window="addNotification($event.detail.message, $event.detail.type)">
        ...
        <template x-for="notification in notifications" :key="notification.id">
            <div x-transition:enter="transition ease-out duration-300"
                 ...
                 :class="{
                    ...
                    'bg-white dark:bg-gray-800 border-red-500 text-red-600 dark:text-red-400': notification.type === 'error',
    ...
    <div class="flex-1 flex flex-col min-w-0">
        ...
        <div class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-8 py-4 flex justify-between items-center shadow-sm z-10">
            <div class="flex items-center space-x-4">
                <div class="bg-brand-red/10 dark:bg-brand-red/20 p-2 rounded-lg">
                    <svg class="w-6 h-6 text-brand-red" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                </div>
                <h1 class="text-2xl font-bold text-gray-800 dark:text-white tracking-tight">Point of Sale</h1>
            </div>
        ...
        <div class="px-8 py-6 overflow-x-auto whitespace-nowrap bg-white dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700/50 backdrop-blur-sm">
            <div class="inline-flex space-x-3">
                <button wire:click="selectCategory(null)" class="px-6 py-2.5 rounded-xl font-semibold text-sm transition-all duration-200 {{ is_null($selectedCategory) ? 'bg-brand-red text-white shadow-lg shadow-brand-red/30 transform scale-105' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-600' }}">
                    All Items
                </button>
                @foreach($categories as $category)
                    <button wire:click="selectCategory({{ $category->id }})" class="px-6 py-2.5 rounded-xl font-semibold text-sm transition-all duration-200 {{ $selectedCategory == $category->id ? 'bg-brand-red text-white shadow-lg shadow-brand-red/30 transform scale-105' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-600' }}">
                        {{ $category->name }}
                    </button>
                @endforeach
            </div>
                ...
                @foreach($products as $product)
                    <div wire:click="addToCart({{ $product->id }})" class="group bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer border border-gray-100 dark:border-gray-700 overflow-hidden relative">
                        <div class="aspect-square overflow-hidden relative bg-gray-100 dark:bg-gray-700">
                            ...
                            @if($product->is_stock_managed)
                                <div class="absolute top-3 right-3">
                                    <span class="px-2.5 py-1 rounded-lg text-xs font-bold shadow-sm backdrop-blur-md {{ $product->stock < 10 ? 'bg-red-500/90 text-white' : 'bg-white/90 dark:bg-gray-800/90 text-gray-800 dark:text-gray-100' }}">
                                        {{ $product->stock }} left
                                    </span>
                                </div>
                            @endif
                        ...
                        <div class="p-4">
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400 mb-3 truncate">{{ $product->category->name ?? 'Uncategorized' }}</p>
    ...
    <div class="w-96 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 flex flex-col shadow-2xl z-20">
        <!-- Customer Info -->
        <div class="p-6 border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 backdrop-blur-sm">
            ...
            <div class="relative" x-data="{ open: false, search: '' }">
                ...
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute w-full mt-2 bg-white dark:bg-gray-700 border border-gray-100 dark:border-gray-600 rounded-xl shadow-xl z-50 max-h-60 overflow-y-auto custom-scrollbar">
                    <div class="p-2 sticky top-0 bg-white dark:bg-gray-700 border-b border-gray-100 dark:border-gray-600">
                        <input x-model="search" type="text" placeholder="Search..." class="w-full px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-800 border-none text-sm focus:ring-1 focus:ring-brand-red text-gray-800 dark:text-white placeholder-gray-500 dark:placeholder-gray-400">
                    </div>
                    <div @click="$wire.selectCustomer(null); open = false" class="px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-600 cursor-pointer text-gray-700 dark:text-gray-200 text-sm font-medium transition-colors">Guest Customer</div>
                    @foreach($customers as $customer)
                        <div x-show="search === '' || '{{ strtolower($customer->name) }}'.includes(search.toLowerCase())" 
                             ...
                             class="px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-600 cursor-pointer text-gray-700 dark:text-gray-200 text-sm flex justify-between items-center transition-colors group">
        ...
        <!-- Cart Items -->
        <div class="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar bg-gray-50/30 dark:bg-gray-900/30">
            @if(empty($cart))
            ...
            @else
                @foreach($cart as $item)
                    <div class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col gap-3 group hover:border-brand-red/30 transition-colors">
                        ...
                        <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 rounded-lg p-1">
                            <button wire:click="updateQuantity({{ $item['id'] }}, {{ $item['quantity'] - 1 }})" class="w-8 h-8 flex items-center justify-center rounded-md bg-white dark:bg-gray-600 text-gray-500 hover:text-brand-red hover:bg-red-50 dark:hover:bg-gray-500 shadow-sm transition-all">
                            ...
                            <button wire:click="updateQuantity({{ $item['id'] }}, {{ $item['quantity'] + 1 }})" class="w-8 h-8 flex items-center justify-center rounded-md bg-white dark:bg-gray-600 text-gray-500 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-gray-500 shadow-sm transition-all">
        ...
        <div class="p-6 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] z-30">
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <button wire:click="$set('payment_method', 'cash')" class="py-2.5 rounded-xl font-bold text-sm border-2 transition-all {{ $payment_method === 'cash' ? 'border-brand-red text-brand-red bg-red-50 dark:bg-red-900/20' : 'border-gray-200 dark:border-gray-700 text-gray-500 hover:border-gray-300 dark:hover:border-gray-600' }}">
                ...
                <button wire:click="$set('payment_method', 'qris')" class="py-2.5 rounded-xl font-bold text-sm border-2 transition-all {{ $payment_method === 'qris' ? 'border-brand-red text-brand-red bg-red-50 dark:bg-red-900/20' : 'border-gray-200 dark:border-gray-700 text-gray-500 hover:border-gray-300 dark:hover:border-gray-600' }}">
                    <span class="flex items-center justify-center">
